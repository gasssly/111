...
import safety_net  # 🛡️ 危機偵測安全網：最高優先級中斷器
import trust_evaluator  # 📊 信任度量化評估：三大指標即時算分
import user_profiler  # 🎭 使用者畫像提取：無痕判斷個性特徵
...
# ==========================================
st.sidebar.markdown("---")
st.sidebar.markdown("📊 **信任度儀表板**")

# 初始化 session state
if "trust_user_turns" not in st.session_state:
    st.session_state.trust_user_turns = []  # 使用者歷史發言清單
if "trust_result" not in st.session_state:
    st.session_state.trust_result = trust_evaluator.compute_trust_score([])
if "user_profile" not in st.session_state:
    st.session_state.user_profile = user_profiler.analyze_profile([])

_tr = st.session_state.trust_result
_up = st.session_state.user_profile

# 信任分數進度條
trust_score = _tr.get("total_score", 0)
st.sidebar.progress(min(trust_score, 100), text=f"信任分數：{trust_score} / 100")
st.sidebar.caption(f"📍 {_tr.get('phase', 'Phase 0')}")

# 三大指標細節
with st.sidebar.expander("🔍 信任指標明細"):
    st.markdown(f"""- **回覆字數**：{_tr.get('length_score', 0)} / 40
- **情緒詞密度**：{_tr.get('emotion_score', 0)} / 30
- **自我揭露**：{_tr.get('disclosure_score', 0)} / 30""")
    if _tr.get("emotion_hits"):
        st.caption(f"偵測到情緒詞：{', '.join(_tr['emotion_hits'][:5])}")
    if _tr.get("disclosure_hits"):
        st.caption(f"偵測到揭露詞：{', '.join(_tr['disclosure_hits'][:5])}")

# 畫像摘要
with st.sidebar.expander("🎭 使用者畫像"):
    coping_label = {"emotion-focused": "情緒導向 💧", "problem-focused": "問題導向 🔧", "unknown": "尚未判定"}
    social_label = {"introvert": "偏內向 🏠", "extrovert": "偏外向 🌳", "unknown": "尚未判定"}
    st.markdown(f"""- **因應型態**：{coping_label.get(_up.get('coping_style', 'unknown'), '尚未判定')}
- **社交傾向**：{social_label.get(_up.get('social_tendency', 'unknown'), '尚未判定')}
- **表達程度**：{_up.get('expression_level', 'silent')}
- **核心議題**：{'、'.join(_up.get('core_concerns', [])) if _up.get('core_concerns') else '尚未判定'}""")
...
def handle_user_input(user_text, image_obj=None):
    ...
    # ==========================================
    # 🛡️ STEP 0：安全網攔截（最高優先級）
    # ==========================================
    if user_text:
        crisis = safety_net.check_safety(user_text)
        if crisis["level"] == 3:
            # Level 3 危機：立即中斷，不送進 LLM
            st.session_state.messages.append({"role": "user", "content": user_text})
            with st.chat_message("user", avatar="🧑‍💻"):
                st.write(user_text)
            crisis_msg = crisis["crisis_response"]
            st.session_state.messages.append({"role": "assistant", "content": crisis_msg})
            with st.chat_message("assistant", avatar="🧠"):
                st.error("⚠️ 阿光偵測到危機訊號，已啟動安全通報機制")
                st.write(crisis_msg)
            # 用語音唸出求助專線
            if st.session_state.get("tts_enabled", True):
                try:
                    audio_bytes = generate_edge_tts_audio("我很擔心你的安全。請你現在就撥打安心專線 1925，會有專業的人陪你。")
    ...
    if user_text:
        # 記錄使用者發言到歷史清單
        st.session_state.trust_user_turns.append(user_text)
        # 即時計算信任分數
        st.session_state.trust_result = trust_evaluator.compute_trust_score(
            st.session_state.trust_user_turns
        )
        # 即時更新使用者畫像
        st.session_state.user_profile = user_profiler.analyze_profile(
            st.session_state.trust_user_turns
    ...
    if "chat" in st.session_state:
        ...
        if past_memory != "" or pdf_knowledge != "":
            ...
            # ── 🆕 信任閘門 + 畫像感知注入 ──
            _trust = st.session_state.get("trust_result", {})
            _profile = st.session_state.get("user_profile", {})
            trust_instruction = trust_evaluator.build_trust_prompt(_trust)
            profile_instruction = user_profiler.build_profile_prompt(_profile)
            
            # ── 安全網 Level 1/2 提示（非中斷，但影響 AI 行為）──
            safety_instruction = ""
            if user_text:
                _crisis = safety_net.check_safety(user_text)
                if _crisis["level"] == 2:
                    safety_instruction = "\n【⚠️ 安全網警戒】偵測到使用者可能有自傷暗示。請切換為純傾聽模式，絕對不要給任何建議，只需要溫柔地陪伴和接住他的情緒。如果情況惡化，請溫柔提示安心專線 1925。\n"
                elif _crisis["level"] == 1:
                    safety_instruction = "\n【💛 安全網留意】偵測到使用者情緒較為低落。請加強同理心深度，多肯定他願意表達的勇氣。\n"
            
            # 顯示 UI 指示器（加入信任分數）
            trust_score_display = _trust.get('total_score', 0)
            trust_phase_display = _trust.get('phase', 'Phase 0')
            st.info(f"🧠 路由：**{route}** 📍 Stage {new_stage} ({DIALOGUE_STAGE_NAMES.get(new_stage, '未知')}) | 📊 信任：{trust_score_display} 分 ({trust_phase_display})")
            ...
            memory_instruction += trust_instruction + "\n" + profile_instruction + "\n" + safety_instruction + stage_instruction + route_instruction + personality_instruction + risk_instruction + f"\n【使用者現在說】：{user_text}"
            ...
            route_instruction = trust_instruction + "\n" + profile_instruction + "\n" + safety_instruction + stage_instruction + route_instruction + personality_instruction + risk_instruction + f"\n【使用者現在說】：{user_text}"
